...
#
#
# @app.route('/api/v1/recommend/<crop>/<input>', method=['GET', 'POST'])
...
#
#
# @app.route('/api/v1/recommend/crop-state')
...
def make_recommendation():
    if request.method == 'POST':
        ...
        return str(list(dict.fromkeys(sorted_similar_crop)))
    else:
...
#         try:
#             crop_dict_value = switching_variables(crop)
#             get_crop_index = getting_state_index(crop_dict_value)
#             similar_crop = list(enumerate(model[get_crop_index]))
#             sorted_similar_crop = sorted(similar_crop, key=lambda x: x[1], reverse=False)
#             return str(list(dict.fromkeys(sorted_similar_crop)))
#         except KeyError:
